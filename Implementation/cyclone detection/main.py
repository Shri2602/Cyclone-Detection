...
def predict():
    ...
    if imagefile and allowed_img(imagefile.filename):
        ...
        # Message logic based on intensity threshold
        hit_message = "99% chances that cyclone will hit" if predicted_intensity >= 34 else "Cyclone will not hit"
        ...
        return render_template('index.html', prediction=predicted_intensity, filename=filename, rain=r, x=1, message=hit_message)
    else:
...
def archive():
    ...
    arranged_records = []
    for row in records:
        coordinates_str = str(row[1])  # Convert datetime.datetime to string
        separator = coordinates_str.find(",")
        lat = coordinates_str[1:separator]
        lng = coordinates_str[separator + 1:-1]

        predicted_intensity = row[2]
        image_filename = row[3]

        # Check if row[4] is a string and convert it to datetime if needed
        if isinstance(row[4], str):
            capture_date = datetime.datetime.strptime(row[4], "%Y-%m-%d").strftime("%d/%m/%Y")
        else:
